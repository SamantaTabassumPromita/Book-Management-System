...
// Create connection
			...
			if (isset($_GET['search']) && !empty($_GET['search'])) {
				$search = $_GET['search'];
 				...
 				$sql = "SELECT * FROM ((users INNER JOIN book ON users.user_ID=book.std_id) INNER JOIN sell ON sell.ISBN = book.ISBN AND sell.std_id1=book.std_id) WHERE Bname LIKE '%$search%' OR book.ISBN LIKE '%$search%' OR auth1 LIKE '%$search%' OR auth2 LIKE '%$search%' OR bdept LIKE '%$search%' OR course LIKE '%$search%' OR type LIKE '%$search%'";
				
				$sql2 ="SELECT * FROM (((users INNER JOIN book ON users.user_ID=book.std_id) INNER JOIN offered ON offered.ISBN = book.ISBN AND offered.std_id3=book.std_id) INNER JOIN exchange_with ON exchange_with.std_id5=offered.std_id3 AND exchange_with.exISBN=offered.exISBN) WHERE Bname LIKE '%$search%' OR book.ISBN LIKE '%$search%' OR book.auth1 LIKE '%$search%' OR book.auth2 LIKE '%$search%' OR bdept LIKE '%$search%' OR course LIKE '%$search%' OR type LIKE '%$search%'";
				$result = mysqli_query($conn, $sql);
				$result2 = mysqli_query($conn, $sql2);
				$num = mysqli_num_rows($result);
				$num2=mysqli_num_rows($result2);
				
				echo "<h4 class='p-3'>For sale: {$num} result(s) for '".$search."'</h4>";
				...
				echo "<h4 class='p-3'>For exchange: {$num2} result(s) for '".$search."' </h4>";
				...
				while ($row2 = mysqli_fetch_assoc($result2)) {
					...
					<div class='col-md-6'>
					<h4 style='display:inline;'> <a href='indbook.php?isbn=".urlencode($row2['exISBN'])."&sid=".urlencode($row2['std_id5'])."'>{$row2['Bname']}</a></h4><p style='display:inline;'> {$row['edition']}th edition</p><br>
