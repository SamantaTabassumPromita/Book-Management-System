...
// Create connection
				...
				$sql ="SELECT * FROM ((users INNER JOIN book ON users.user_ID=book.std_id) INNER JOIN sell ON sell.ISBN = book.ISBN AND sell.std_id1=book.std_id) WHERE book.bdept = '$catdept'";
				$sql2 ="SELECT * FROM (((users INNER JOIN book ON users.user_ID=book.std_id) INNER JOIN offered ON offered.ISBN = book.ISBN AND offered.std_id3=book.std_id) INNER JOIN exchange_with ON exchange_with.std_id5=offered.std_id3 AND exchange_with.exISBN=offered.exISBN) WHERE book.bdept = '$catdept'";
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					<div class='col-md-6'>
					<h4 style='display:inline;'> <a href='indbook.php?isbn=".urlencode($row['ISBN'])."&sid=".urlencode($row['std_id1'])."'>{$row['Bname']}</a></h4><p style='display:inline;'> {$row['edition']}th edition</p>\n
